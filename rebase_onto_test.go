...
func TestRebaseOntoCmd_HappyPath(t *testing.T) {
	...
	// Now run the rebase onto command
	rebaseOntoCmd := &RebaseOntoCmd{
		NewBase: "main",
	}

	// Mock user input to confirm the rebase onto operation
	restoreStdin := mockInput("y")
	defer restoreStdin()

	err = rebaseOntoCmd.Run(nil)
	require.NoError(t, err, "Rebase onto command should succeed")
...
// TestRebaseOntoCmd_SameBase tests rebasing onto the same base (should be a no-op)
func TestRebaseOntoCmd_SameBase(t *testing.T) {
	repoPath, repo, cleanup := setupTestEnv(t)
	defer cleanup()
	...
	err = rebaseOntoCmd.Run(nil)
	require.NoError(t, err, "Rebase onto same base should succeed")
...
// TestRebaseOntoCmd_ErrorCases tests various error conditions
func TestRebaseOntoCmd_ErrorCases(t *testing.T) {
	tests := []struct {
		name        string
		setup       func(t *testing.T, repo *git.Repository, repoPath string) *RebaseOntoCmd
		expectError string
	}{
		{
			...
			setup: func(t *testing.T, repo *git.Repository, repoPath string) *RebaseOntoCmd {
				...
				return &RebaseOntoCmd{NewBase: "nonexistent-branch"}
			},
			expectError: "failed to resolve rebase onto base 'nonexistent-branch'",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rebaseOntoCmd := tt.setup(t, repo, repoPath)
...
// TestRebaseOntoCmd_CommitHash tests rebasing onto a commit hash instead of branch name
func TestRebaseOntoCmd_CommitHash(t *testing.T) {
	repoPath, repo, cleanup := setupTestEnv(t)
	defer cleanup()
	...
	err = rebaseOntoCmd.Run(nil)
	require.NoError(t, err, "Rebase onto commit hash should succeed")
...
// TestRebaseOntoCmd_WithConflicts tests rebase onto when modify/delete conflicts occur during cherry-pick
func TestRebaseOntoCmd_WithConflicts(t *testing.T) {
	repoPath, repo, cleanup := setupTestEnv(t)
	defer cleanup()
...
// TestRebaseOntoCmd_NoBase tests rebase onto when tower has no base set
func TestRebaseOntoCmd_NoBase(t *testing.T) {
	repoPath, repo, cleanup := setupTestEnv(t)
	defer cleanup()
	...
	// Mock user input to confirm the rebase onto operation (if it gets that far)
	restoreStdin := mockInput("y")
	defer restoreStdin()
...
// TestRebaseOntoCmd_SavesUndoState tests that rebase onto saves undo state before making changes
func TestRebaseOntoCmd_SavesUndoState(t *testing.T) {
	repoPath, repo, cleanup := setupTestEnv(t)
	defer cleanup()
	...
	err = rebaseOntoCmd.Run(nil)
	require.NoError(t, err, "Rebase onto should succeed")
...
// TestRebaseOntoCmd_Undo tests the rebase onto undo functionality
func TestRebaseOntoCmd_Undo(t *testing.T) {
	repoPath, repo, cleanup := setupTestEnv(t)
	defer cleanup()
	...
	rebaseUndoCmd := &RebaseUndoCmd{}

	err = rebaseUndoCmd.Run(nil)
